...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      // 練習ToDo(アプリ全体で1つ、id=1固定行にまるごとJSONを保存)
      if (path === '/api/todos' && request.method === 'GET') {
        if (!requireKey()) return err('invalid key', 401);
        const row = await env.DB.prepare('SELECT items_json, updated_at FROM practice_todos WHERE id = 1').first();
      ...
      if (path === '/api/todos' && request.method === 'PUT') {
        ...
        await env.DB.prepare(
          `INSERT INTO practice_todos (id, items_json, updated_at) VALUES (1, ?, ?)
